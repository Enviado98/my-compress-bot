...
task_queue = deque()
is_processing = False
cancel_tasks = set()  # Para cancelar tareas por chat
...
def handle_buttons(update: Update, context: CallbackContext):
    ...
    if data == "compress":
        ...
        cancel_tasks.add(chat_id)
        query.edit_message_text("❌ Tu tarea ha sido cancelada.")
...
def process_task_queue(context: CallbackContext):
    ...
    while task_queue:
        ...
        def progress_callback(pct):
            pct_rounded = min(100, int(pct // 25 * 25))
            try:
                context.bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text=f"Procesando tu video: {pct_rounded}%")
            except:
                pass
        ...
        try:
            if task_type == "compress":
                ...
                compress_video(input_path, output_path, quality=quality, progress_callback=progress_callback)
                context.bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text="✅ Video comprimido listo!")
                with open(output_path, "rb") as f:
                    context.bot.send_video(chat_id, f)
            elif task_type == "extract_audio":
                output_path = f"audio_{input_path}.mp3"
                extract_audio(input_path, output_path, progress_callback=progress_callback)
                context.bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text="✅ Audio extraído listo!")
                with open(output_path, "rb") as f:
                    context.bot.send_audio(chat_id, f)
        except Exception as e:
            context.bot.send_message(chat_id, text=f"⚠️ Tarea cancelada o error: {str(e)}")
